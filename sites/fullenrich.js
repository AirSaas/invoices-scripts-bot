  ...
  try {
    ...
    try {
      ...
      if (currentUrl.includes('login') || currentUrl.includes('auth') || currentUrl.includes('signin')) {
        throw new Error('Page requires authentication. Please login to Fullenrich first.');
      }
    ...
    const fs = require('fs');
    const debugFilteredPath = path.resolve(__dirname, '..', 'logs', `fullenrich-filtered-${Date.now()}.html`);
    ...
    if (candidates.length === 0) {
      ...
      const debugPath = path.resolve(__dirname, '..', 'logs', `fullenrich-filtered-debug-${Date.now()}.html`);
      ...
      const originalPath = path.resolve(__dirname, '..', 'logs', `fullenrich-original-debug-${Date.now()}.html`);
